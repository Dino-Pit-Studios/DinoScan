...
class DuplicateCodeAnalyzer(ASTAnalyzer):
    ...
    def _extract_code_blocks(self, file_path: str) -> list[CodeBlock]:
        ...
        for block in extractor.blocks:
            if self._is_valid_block(block):
                ...
                block.similarity_hash = self._compute_similarity_hash(block.normalized_content)
                blocks.append(block)
    ...
    def _find_duplicates_in_blocks(self, blocks: list[CodeBlock], within_file: bool) -> list[Finding]:
    ...
    def _create_exact_duplicate_findings(self, duplicate_blocks: list[CodeBlock]) -> list[Finding]:
        ...
        for i in range(len(duplicate_blocks)):
            for j in range(i + 1, len(duplicate_blocks)):
                ...
                if (block1.file_path == block2.file_path and 
                    abs(block1.start_line - block2.start_line) < 3):
                    continue
    ...
    def _find_similar_blocks(self, blocks: list[CodeBlock], within_file: bool) -> list[Finding]:
            ...
            for j in range(i + 1, len(blocks)):
                ...
                if similarity >= self.similarity_threshold:
                    ...
                    finding = Finding(
                        ...
                        tags={"duplicate", match_type, f"{similarity:.0%}-similar", f"{line_count}-lines"}
                    )
                    findings.append(finding)
  ...
  %(prog)s /path/to/project --output-format json --output-file duplicates.json
    ...
    try:
        ...
        if args.verbose:
            ...
            sys.stderr.write(f"Analysis complete: {stats['total_findings']} duplicate code issues found\n")
