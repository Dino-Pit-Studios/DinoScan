...
class CircularImportAnalyzer(ASTAnalyzer):
    ...
    def _resolve_import(
        ...
        if import_info.import_type == 'relative':
            return self._resolve_relative_import(import_info, current_file, project_path)
        else:
    ...
    def _resolve_relative_import(
        ...
        target_parts = current_parts[:-import_info.level] if import_info.level > 0 else current_parts
    ...
    def _find_import_line(self, module_info: ModuleInfo, target_module: str) -> int:
        ...
        for import_info in module_info.imports:
            ...
            if (import_info.module_name == target_module or
                target_module.startswith(import_info.module_name + '.')):
  ...
  %(prog)s /path/to/project --include-external --max-depth 5
    ...
    try:
        ...
        if args.verbose:
            ...
            print(f"Analysis complete: {stats['total_findings']} circular imports found")
