...
class DeadCodeAnalyzer(ASTAnalyzer):
    ...
    def _create_dead_code_finding(self, symbol: Symbol) -> Finding | None:
        ...
        if symbol.is_private or (
            symbol.symbol_type == "function" and symbol.decorators
        ):
    ...
    def _get_removal_suggestion(self, symbol: Symbol) -> str:
        ...
        base_suggestion = suggestions.get(
            symbol.symbol_type, f"Remove unused {symbol.symbol_type} '{symbol.name}'"
        )
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Name) and node.func.id in self.symbols:
            ...
            if (
                ...
                and node.func.value.id in self.symbols
            ):
                usage = Usage(
  ...
  %(prog)s /path/to/project --include-tests --aggressive
    ...
    try:
        ...
        if args.verbose:
            ...
            sys.stderr.write(
                f"Analysis complete: {stats['total_findings']} dead code issues found\n"
            )
