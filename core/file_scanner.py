...
class FileScanner:
    ...
    def _load_exclusions(self) -> None:
        ...
        self.excluded_dirs = set(exclusions.get('directories', [
            '__pycache__', '.git', '.pytest_cache', '.mypy_cache',
            'venv', 'env', '.env', 'node_modules', 'build', 'dist',
            '.vscode', '.idea', 'htmlcov', 'coverage', '.tox', '.nox'
        ]))
        
        self.excluded_file_patterns = exclusions.get('files', [
            '*.pyc', '*.pyo', '*.pyd', '*.so', '*.dll', '*.exe',
            '*.log', '*.tmp', '*.cache', '*.lock', '*.pid',
            '*.min.js', '*.min.css', '*.bundle.js', '*.bundle.css'
        ])
        
        self.supported_extensions = set(exclusions.get('extensions', [
