...
from .base_analyzer import ASTAnalyzer, Finding, Severity, Category
...
class PythonASTVisitor(ast.NodeVisitor):
    ...
    def add_finding(
        ...
        finding = Finding(
            ...
            end_column=getattr(node, 'end_col_offset', 0) or getattr(node, 'col_offset', 0),
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                elif not self._is_constant_context(node) and not self._is_snake_case(var_name):
                    self.add_finding(
